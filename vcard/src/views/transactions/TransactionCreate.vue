<template>
    ...
    <div class="text-center mt-5">
      ...
      <div class="container">
        <div class="form-group" style="display: flex; flex-direction: column" v-show="loadedPaymentTypes">
            <label for="selectPaymentType"><b>Payment Type</b></label>
            <select v-model="payment_type" name="payment_type_id" id="selectPaymentType" class="form-select">
                <option v-for="payment in payment_types" :key="payment.code" :value="payment.code" >
                    {{payment.name}}
                </option>
            </select>
        ...
        <input type="text" v-model="payment_reference" placeholder="Enter Payment reference" name="payment_ref" required>
        <div v-show="errors.payment_reference != undefined" class="text-danger">{{errors.payment_reference}}</div>
        <div v-show="errors.pair_vcard != undefined" class="text-danger">{{errors.pair_vcard}}</div>
        <label for="value"><b>Value:</b></label>
        <input type="number" v-model="value" class="form-control" placeholder="Enter Value" name="value" required/>
      ...
      this.$axios
        ...
        .then(() => {
          ...
          this.$router.push({
            name: "dashboard",
            params: {
              successMessage: "Transaction created successfully (Click me to close)",
            },
          });
        })
        .catch((error) => {
