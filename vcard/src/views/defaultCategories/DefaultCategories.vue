  ...
  <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    <div class="container d-flex flex-row justify-content-between">
      ...
      <router-link
        ...
        :to="{ name: 'defaultCategoriesCreate' }"
        >New Default Category</router-link
      >
    </div>
    <nav
...
export default {
  ...
  methods: {
    deleteCategory(id) {
      this.$axios
        .delete(`defaultCategories/${id}`)
        .then((response) => {
          this.$toast.info(
            `Default Category ${response.data.data.name} removed`
          );
  ...
  computed: {
    changesAdminDefaultCategories() {
      return this.$store.getters.changesAdminDefaultCategories;
    },
  },
  watch: {
    changesAdminDefaultCategories: {
      handler() {
        if (this.$store.getters.changesAdminDefaultCategories)
          this.getDefaultCategories();
      },
      deep: true,
    },
